 ...
 */
@WebServlet(name = "RiceHarvesting", urlPatterns = {"/RiceHarvesting"})
public class RiceHarvesting extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean status = dao.saveRiceHarvesting(vo);
            HttpSession session=request.getSession();
            if(status == true){
                session.setAttribute("Message", "Successfully Saved");
                response.sendRedirect("harvesting_rice.jsp");
            } else {
                response.sendRedirect("harvesting_rice.jsp");
            }
        } catch (Exception e) {
