 ...
 */
@WebServlet(name = "SoyabeanHarvesting", urlPatterns = {"/SoyabeanHarvesting"})
public class SoyabeanHarvesting extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean status = dao.saveSoyabeanHarvesting(vo);
            HttpSession session=request.getSession();
            if(status == true){
                session.setAttribute("Message", "Successfully Saved");
                response.sendRedirect("harvesting_soya.jsp");
            } else {
                response.sendRedirect("harvesting_soya.jsp");
            }
        } catch (Exception e) {
