 ...
 */
@WebServlet(name = "MaizeHarvesting", urlPatterns = {"/MaizeHarvesting"})
public class MaizeHarvesting extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean status = dao.saveMaizeHarvesting(vo);
            HttpSession session=request.getSession();
            if(status == true){
                session.setAttribute("Message", "Successfully Saved");
                response.sendRedirect("harvesting_maize.jsp");
            } else {
                response.sendRedirect("harvesting_maize.jsp");
            }
        } catch (Exception e) {
