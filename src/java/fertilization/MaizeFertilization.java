 ...
 */
@WebServlet(name = "MaizeFertilization", urlPatterns = {"/MaizeFertilization"})
public class MaizeFertilization extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter())
        {
            ...
            boolean status = dao.saveMaizeFertilization(vo);
            
            HttpSession session=request.getSession();
            if(status == true){
                session.setAttribute("Message", "Successfully Saved");
                response.sendRedirect("fertilization_maize.jsp");
            } else {
                response.sendRedirect("fertilization_maize.jsp");
            }
        }
